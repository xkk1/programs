...
//定义地区数组
var city_array = { 11: "北京", 12: "天津", 13: "河北", 14: "山西", 15: "内蒙古", 21: "辽宁", 22: "吉林", 23: "黑龙江", 31: "上海", 32: "江苏", 33: "浙江", 34: "安徽", 35: "福建", 36: "江西", 37: "山东", 41: "河南", 42: "湖北", 43: "湖南", 44: "广东", 45: "广西", 46: "海南", 50: "重庆", 51: "四川", 52: "贵州", 53: "云南", 54: "西藏", 61: "陕西", 62: "甘肃", 63: "青海", 64: "宁夏", 65: "新疆", 71: "台湾", 81: "香港", 82: "澳门", 91: "国外" }
...
/**
 * @function 验证身份证号是否正确
 * @description 返回空字符串为正确
 * @param {String} id_str 传入名称
 * @return {String}
 * @author 小喾苦
 * @example verify_id("320302196404272039")
 */
verify_id = (id_str) => {
    // 验证身份证号位数
    if (id_str.length === 15) {
        return "仅支持18位身份证号码，不支持15位身份证号码";
    }
    if (id_str.length !== 18) {
        return `身份证号位数不对，应为18位，不应为${id_str.length}位`;
    }
    // 身份证格式是否正确
    if (/(^\d{17}(\d|X|x)$)/.test(id_str) === false) {
        return "身份证号格式有问题，请重新输入";
    }
    if (id_str[id_str.length - 1] === "x") {
        return "身份证号最后一位X为大写";
    }
    // 检查生日日期是否正确
    var reg = new RegExp(/^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/);
    var arrSplit = id_str.match(reg);
    if (!arrSplit) {
		return "身份证号格式有问题，请重新输入";
	}
	...
	if (!bGoodDay) {
		...
		var nTemp = 0, i;
		for (i = 0; i < 17; i++) {
			nTemp += id_str.slice(i, 1+i) * arrInt[i];
		}
		valnum = arrCh[nTemp % 11];
		if (valnum != id_str.slice(17, 18)) {
			return "18位身份证的校验码不正确！应该为：" + valnum;
		}
    }
    // 检查地区是否合法
    if (city_array[parseInt(id_str.slice(0, 2))] == null) {
        return "非法地区！";
    }
    return "";
}

get_verify_id = () => {return verify_id($("#id-input").value)};

query_id = () => {
    let msg = get_verify_id();
    if (msg !== "") {
        Notiflix.Report.failure('查询失败', '查询失败！\n' + msg, '确定',);
    } else {
        let id_str = $("#id-input").value;
        var reg = new RegExp(/^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/);
        var arrSplit = id_str.match(reg);
        if (!arrSplit) {
            Notiflix.Report.failure('查询失败', '查询失败！\n身份证号格式有问题，请重新输入', '确定',);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $("#id-input").onkeyup = () => {
        if (window.event.keyCode == 13) {
            ...
            $("#msg").textContent = get_verify_id();
        }
    };
    $("#query_id_btn").onclick = query_id;
});
